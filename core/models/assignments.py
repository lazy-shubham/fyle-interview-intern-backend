...
class Assignment(db.Model):
    ...
    def upsert(cls, assignment_new: 'Assignment'):
        if assignment_new.id is not None:
            ...
            assertions.assert_valid(
                assignment.student_id == assignment_new.student_id,
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        assignment = Assignment.get_by_id(_id)
        teacher_id = getattr(auth_principal, "teacher_id")
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(grade is not None, 'assignment with empty grade cannot be graded')
        assertions.assert_valid(
            assignment.state != AssignmentStateEnum.DRAFT.value, 
            "assignment in draft state cannot be graded"
        )
        if teacher_id:
            assertions.assert_valid(
                assignment.teacher_id == teacher_id,
                "assignment not submitted to you"
            )
            assertions.assert_valid(
                assignment.state == AssignmentStateEnum.SUBMITTED.value,
    ...
    def get_submitted_or_graded_assignments(cls):
        return cls.filter(cls.state != AssignmentStateEnum.DRAFT.value).all()
